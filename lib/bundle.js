...
/***/ function(module, exports, __webpack_require__) {
	...
	function paintNeighbors(row, col, color) {
	  if (row < n_rows - 1) testColorFlood(row + 1, col, color);
	  if (row > 0) testColorFlood(row - 1, col, color);
	  if (col < n_cols - 1) testColorFlood(row, col + 1, color);
	  if (col > 0) testColorFlood(row, col - 1, color);
	}
	...
	function paint(color, initial) {
	  if (lost === true) {
	    ...
	    var prev_color = gameBoard[0][0].color;
	    if (!initial && color == prev_color) return;
	    moves++;
	    ...
	    for (var _row = 0; _row < n_rows; _row++) {
	      for (var _col = 0; _col < n_cols; _col++) {
	        if (gameBoard[_row][_col].painted) {
	          paintNeighbors(_row, _col, color);
	        }
	      }
	    }
	    if (allPainted()) {
	...
	function createSequence(size) {
	  var tempo = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "4n";
	    ...
	    var _loop = function _loop(row) {
	      if (gameBoard[row][col].activated) {
	        (function () {
	          ...
	          var squareColor = gameBoard[row][col].color;
	          switch (squareColor) {
	            case "red":
	...
	function updateColors() {
	  colors.forEach(function (color) {
	    $("." + color + "-button").removeClass("active-color-button");
	  });
	
	  activatedColors.forEach(function (color) {
	    switch (color) {
	      case "red":
	        $(".red-button").addClass("active-color-button");
	        break;
	      case "yellow":
	        $(".yellow-button").addClass("active-color-button");
	        break;
	      case "blue":
	        $(".blue-button").addClass("active-color-button");
	        break;
	      case "orange":
	        $(".orange-button").addClass("active-color-button");
	        break;
	      case "purple":
	        $(".purple-button").addClass("active-color-button");
	        break;
	      case "green":
	        $(".green-button").addClass("active-color-button");
	        break;
	      default:
	        break;
	    }
	...
	function updateOnOff() {
	  if (playing) {
	    $(".start-stop").addClass("clicked");
	  } else {
	    $(".start-stop").removeClass("clicked");
	  }
	}
	...
	function mosaic() {
	  ...
	  var color2 = document.getElementById("mosaic-color-2").value;
	  if (!color1 || !color2) {
	    return;
	  }
	  for (var row = 0; row < n_rows; row++) {
	...
	function wackyPainterModeToggle() {
	  if (wackyPainterMode) {
	    $(".wacky-button").addClass("clicked");
	  } else {
	    $(".wacky-button").removeClass("clicked");
	  }
	}
	
	function randomColors() {
	  $('#gameBoard').on("mouseenter", ".square", function (e) {
	    var $sq = $(e.currentTarget);
	    var className = $sq.attr("class").split(" ");
	    var row = className[2];
	    var col = className[3];
	    var color = randomColor();
	    $sq.removeClass();
	    var klass = "square " + color + " " + row + " " + col;
	...
	function painterModeToggle() {
	  if (painterMode) {
	    $(".painter-button").addClass("clicked");
	  } else {
	    $(".painter-button").removeClass("clicked");
	  }
	}
	...
	function selectColors() {
	  $('#gameBoard').on("click", ".square", function (e) {
	    ...
	    $sq.removeClass();
	    if (className[1] !== "gray") {
	      klass = "square gray" + " " + row + " " + col;
	      gameBoard[row][col].element.className = klass;
	      gameBoard[row][col].color = "gray";
	    } else {
	      klass = "square " + painterColor + " " + row + " " + col;
	      gameBoard[row][col].element.className = klass;
	      gameBoard[row][col].color = painterColor;
	    }
	    toggleSquares();
	  });
	}
	
	function updatePainterColor() {
	  var color = document.getElementById("painter-color").value;
	  painterColor = color;
	}
	...
	function showRestart() {
	  debugger;
	  if (moves < maxMoves) {
	    $(".winner-button").addClass("show-button");
	  } else {
	    $(".loser-button").addClass("show-button");
	  }
	}
	
	function restartGame() {
	  lost = false;
	  $(".winner-button").removeClass("show-button");
	  $(".loser-button").removeClass("show-button");
	  newGame();
	}
	// Start Game
	...
	document.addEventListener("DOMContentLoaded", function () {
	  ...
	  window.updatePainterColor = updatePainterColor;
